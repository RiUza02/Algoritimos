...
void ArvBin::contagens(int k)
{
    int Impar = 0;
    int FilhosEsq = 0;
    auxcontagens(raiz, &Impar, &FilhosEsq, k);

    cout << endl << "Quantidade de elementos com o seu valor impar, com o mesmo acima do nivel informado: " << Impar << endl;
    cout << "Quantidade de elementos com um unico filho, com o mesmo a esquerda e ate o nivel informado: " << FilhosEsq << endl;
}
void ArvBin::auxcontagens(NoArv *p, int *Impar, int *FilhosEsq, int k)
{
    if(p != NULL && k >= 0)
    {
        auxcontagens(p->getEsq(), Impar, FilhosEsq, k-1);
        auxcontagens(p->getDir(), Impar, FilhosEsq, k-1);

        if((p->getEsq() != NULL) && (p->getDir() == NULL))
        {
            *FilhosEsq = *FilhosEsq + 1;
        }
        if((p->getInfo()%2!=0) && (k>0))
        {
            *Impar = *Impar + 1;
        }
    }
