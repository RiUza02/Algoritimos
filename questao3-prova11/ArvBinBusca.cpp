...
ArvBinBusca* ArvBinBusca::copiaValores(int a, int b)
{
    ArvBinBusca *NovaArv = new ArvBinBusca;
    auxCopiaValores(raiz, a, b, NovaArv);
    return NovaArv;
}
void ArvBinBusca::auxCopiaValores(NoArv *p, int a, int b, ArvBinBusca *Arv)
{
    if(p != NULL)
    {
        if(p->getInfo() < a)
        {
            auxCopiaValores(p->getDir(), a, b, Arv);
        }
        else
        {
            if (p->getInfo() > b)
            {
                auxCopiaValores(p->getEsq(), a, b, Arv);
            }
            else
            {
                Arv->insere(p->getInfo());
                auxCopiaValores(p->getEsq(), a, b, Arv);
                auxCopiaValores(p->getDir(), a, b, Arv);
            }
        }
